...
<html lang="en">
    ...
    <body>          
        ...
        <?php $path = explode('/',$_SERVER["REQUEST_URI"]); //the current path
            $sierpinski_image = '/templates/sierpinski/sierpinski.jpg'; //the default
            $sierpinski_main_image_exists = file_exists('./posts/sierpinski.jpg'); //with the dot, because it has to be an absolute path
            $sierpinski_post_image_exists = file_exists($image); //Post image already defaults to profile, so not that
            $sierpinski_main_image = '/posts/sierpinski.jpg'; //without the dot, because the CSS won't work with it
            if($sierpinski_main_image_exists) {
                $sierpinski_image = $sierpinski_main_image; //if we have a post image, override it with that
            }
            $sierpinski_is_home = (count(array_filter($path)) == 0); //if there's nothing in the exploded path
        if($sierpinski_is_home) { ?>                        
           
            <div id="background" style="background: url(<?php echo($sierpinski_image) ?>) no-repeat center center fixed;"></div>
            <div id="hero">
                <div id="hero-content">
        ...
        <?php } else if($sierpinski_post_image_exists) { //if it's an image page ?>
        ...
        <div id="main">
            ...
            <article class="intro">
                ...
                <div class="look0" style="background-image: url('<?php echo($post_image); ?>');">
                </div>
